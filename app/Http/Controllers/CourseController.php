...
class CourseController extends Controller
{
    ...
    {
        $user = Auth::user();
        $isEnrolled = $user ? $course->isUserEnrolled($user) : false;
        ...
        if ($section) {
            ...
            // If no specific section, get the first one
            $currentSection = $sections->first();
        }

        // Get progress only for enrolled users
        $progress = $isEnrolled ? $course->getProgressForUser($user) : 0;
        $completedSections = $isEnrolled ? $course->getCompletedSectionsForUser($user) : [];
     ...
     */
    public function completeCourseAndGenerateCertificate(Course $course)
    {
        try {
            $user = auth()->user();
            ...
            if (!$course->isPurchasedBy($user)) {
                throw new \Exception('You must be enrolled in this course to complete it.');
            }

            // Get all section IDs for this course
            $courseSectionIds = $course->sections->pluck('id')->toArray();

            // Get completed section IDs for this user
            $completedSectionIds = json_decode($course->users()
            ...
            if (count(array_diff($courseSectionIds, $completedSectionIds)) > 0) {
            ...
            $certificate = $this->generateCertificate($course, $user);

            return response()->json([
                'success' => true,
                'message' => 'Course completed successfully!',
                'certificate_url' => route('courses.certificate', ['course' => $course])
            ]);
    ...
    public function completeCourse(Course $course)
    {
        ...
        // Check if user is enrolled
        if (!$course->isPurchasedBy($user)) {
            return back()->with('error', 'You must be enrolled in this course to complete it.');
        }
        ...
        // Check if all sections are completed
        $allSectionsCompleted = !array_diff($courseSectionIds, $completedSectionIds);

        if (!$allSectionsCompleted) {
            return back()->with('error', 'You must complete all sections before completing the course.');
        }
